...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden" style="box-shadow:0 1px 3px rgba(0,0,0,.05)">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-slate-800 text-sm">Edit Produk</h2>
                <p class="text-xs text-slate-400 mt-0.5">{{ $product->product_name }}</p>
            </div>
            <span class="text-xs text-slate-400 bg-slate-100 px-2.5 py-1 rounded-lg font-mono">ID #{{ $product->id }}</span>
        </div>
        <div class="p-6">
