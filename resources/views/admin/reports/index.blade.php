...
<div class="bg-white rounded-2xl border border-slate-100 overflow-hidden" style="box-shadow:0 1px 3px rgba(0,0,0,.05)">
    <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
        <div>
            <h3 class="font-bold text-slate-800 text-sm">Semua Pesanan</h3>
            <p class="text-xs text-slate-400 mt-0.5">{{ $bulanFull[$month-1] }} {{ $year }}</p>
        </div>
        <span class="text-xs font-semibold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg">
            ...
            <tbody>
                @forelse($orders as $order)
                    <tr class="border-b border-slate-50 hover:bg-slate-50/70 transition-colors">
                        ...
                        <td class="px-6 py-3.5">
                            @php
                                $badge = match($order->status) {
                                    'delivered','completed' => 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200',
                        ...
                        <td class="px-6 py-3.5 text-xs text-slate-400">{{ $order->created_at->format('d M Y, H:i') }}</td>
                        <td class="px-6 py-3.5">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-14 text-center">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <p class="text-sm text-slate-400">Tidak ada pesanan bulan ini</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    @if($orders->hasPages())
        <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50">{{ $orders->links() }}</div>
    @endif
</div>
